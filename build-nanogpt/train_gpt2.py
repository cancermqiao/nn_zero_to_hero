...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        # mask matrics, following the OpenAI/HF naming
    ...
    def forward(self, x):
        ...
        # multi head q, k, v in all
        # C = n_embedding = n_head * hs
        # e.g. in GPT-2 (124M), n_head=12, hs=64, C=n_embedding=768
        qkv = self.c_attn(x)
...
class GPT(nn.Module):
    ...
    def from_pretrained(cls, model_type):
        """Loads pretained GPT-2 model weights from huggingface"""
        assert model_type in {'gpt2', 'gpt2-medium', 'gpt2-large', 'gpt2-xl'}
        from transformers import GPT2LMHeadModel
        print(f"loading weights from pretained gpt: {model_type}")
        ...
        # pytorch reverse the weight dimension for calculation performance
